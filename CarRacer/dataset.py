...
class LSTMReplayDataset(torch.utils.data.IterableDataset):

    def __init__(
        self,
        replay_dir: Path = config.REPLAY_DIR,
        z_dir: Path = config.Z_DIR,
        test: bool = False,
        split_percent: float = 0.8,
        max_length: int = config.MAX_STEPS,
    ):
        super().__init__()
...
def lstm_collate_fn(data):
    ...
    dummy_z = torch.zeros((config.MAX_STEPS, hidden_states[0].shape[-1]),
                          dtype=torch.float32)
    dummy_a = torch.zeros((config.MAX_STEPS, actions[0].shape[-1]),
                          dtype=torch.float32)
    ...
    padded_z = padded_z[:, :-1]
    padded_actions = padded_actions[:, :-1]
