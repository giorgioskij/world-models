...
def load_memory(filename: str = "memory.pt") -> M:
    m = M(**config.LSTM).to(config.DEVICE)
    m.load_state_dict(torch.load(config.CKP_DIR / filename))
    return m


if __name__ == "__main__":
    lstm_args = config.LSTM
    m = M(**lstm_args)
    ...
    # print(lstm)
    # # summary(lstm)
    # summary(lstm, (1, 1000, 35))
    i = torch.rand(10, 2, 35)
    o = m(i)
    target = torch.rand(10, 2, 32)
    # loss = mdn.mdn_loss(*o, target)
