...
def run_epoch(
    ...
    for i, data in enumerate(dataloader):
        ...
        zero_vector = torch.zeros(1,
                                  ...
                                  unpacked_data.shape[2] - cfg.NUM_ACTIONS,
                                  device=dev)
        target = torch.cat(
            (unpacked_data[1:, :, :-cfg.NUM_ACTIONS], zero_vector), dim=0)
...
def train_memory(
    ...
    for epoch in range(num_epochs):
        ...
        if test_loss < best_loss:
            best_loss = test_loss
            torch.save(m.state_dict(), cfg.CKP_DIR / "memory.pt")
...
def test_memory(m: memory.M):
    ...
    loss = run_epoch(m, test_dataloader, train=False)
